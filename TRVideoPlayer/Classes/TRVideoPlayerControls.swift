...
class TRVideoPlayerControls: UIView {
    ...
    var delegate: TRVideoPlayerControlsDelegate?
    
    /// custom header which comes as a default header
    var videoPlayerHeader: TRVideoPlayerHeaderView?
    ...
    func createOverlayViewWith(configuration: TRConfiguration, theme: TRTheme, header: UIView?) {
        ...
        if configuration.canShowHeader {
            if header == nil {
                videoPlayerHeader = TRVideoPlayerHeaderView(configuration: configuration, theme: theme, delegate: delegate)
    ...
    private func applyTheme(_ theme: TRTheme) {
                ...
                DispatchQueue.main.async {[weak self] in
                    ...
                    if newStatus == .playing || newStatus == .paused {
                        if let player = self.delegate?.playerStateDidChange {
                            player((self.isActive ? TRVideoPlayerState.pause : TRVideoPlayerState.playing))
                        }
...
extension TRVideoPlayerControls: UICollectionViewDataSource {
...
extension TRVideoPlayerControls: UICollectionViewDelegate {
